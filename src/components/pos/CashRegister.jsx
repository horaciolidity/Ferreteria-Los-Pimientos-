...
const CashMovementDialog = ({ isOpen, onOpenChange }) => {
  ...
  const handleAddMovement = () => {
    ...
    // IMPORTANT: el reducer espera "concept"
    dispatch({
...
export default function CashRegister() {
  ...
  const {
    ...
    isOpen = false,
    openedAt,
    // Este campo lo vamos a rellenar al tocar el reducer (POSContext). Por ahora puede venir 0.
    cashFromMixed = 0,
  } = cr;
  ...
  // Esperado (EFECTIVO): apertura + ventas cash + efectivo de mixto + movimientos
  const expectedAmount = Number(currentOpening || 0) + Number(totalCashSales || 0) + Number(cashFromMixed || 0) + Number(movNet || 0);
  const difference = Number(currentAmount || 0) - Number(expectedAmount || 0);

  // Totales del Turno (openedAt -> ahora) SOLO VENTAS reales (excluye presupuestos)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-glass p-6 rounded-lg">
          ...
          {isOpen ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Abierta el: {openedAt ? new Date(openedAt).toLocaleString() : '-'}</p>
                  </div>
                </div>
              ...
              <div className="space-y-2 text-base">
                <div className="flex justify-between text-muted-foreground">
                  <span>Monto Inicial</span><span className="font-medium">${Number(currentOpening).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Ventas (Efectivo)</span><span className="font-medium">${Number(totalCashSales).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Ventas (Transf.)</span><span className="font-medium">${Number(totalTransferSales).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Ventas (Mixto - Total)</span><span className="font-medium">${Number(totalMixedSales).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Mixto (Parte en efectivo)</span><span className="font-medium">${Number(cashFromMixed || 0).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Movimientos</span>
                  <span className={`font-medium ${movNet >= 0 ? 'text-green-500' : 'text-red-500'}`}>${Number(movNet).toFixed(2)}</span>
                </div>

                <div className="flex justify-between text-foreground text-lg font-bold border-t border-border pt-2 mt-2">
                  <span>Monto Esperado (Efectivo)</span><span>${Number(expectedAmount).toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-foreground text-2xl font-bold">
                  <span>Monto Actual (Efectivo)</span><span>${Number(currentAmount).toFixed(2)}</span>
                </div>
                <div className={`flex justify-between text-lg font-semibold ${Number(difference) === 0 ? 'text-green-600' : 'text-red-600'}`}>
                  <span>Diferencia</span><span>${Number(difference).toFixed(2)}</span>
                </div>
              ...
              <Button onClick={handleCloseRegister} className="w-full bg-red-600 hover:bg-red-700 h-12 text-base">
            ...
            <div className="space-y-4">
              ...
              <Button onClick={handleOpenRegister} className="w-full bg-green-600 hover:bg-green-700 h-12 text-base">
        ...
        <div className="card-glass p-6 rounded-lg">
          ...
          {!isOpen ? (
            ...
            <div className="space-y-2 text-base">
              <div className="flex justify-between"><span>Subtotal (sin IVA)</span><span className="font-medium">${Number(subtotalTurno).toFixed(2)}</span></div>
              <div className="flex justify-between"><span>IVA</span><span className="font-medium">${Number(ivaTurno).toFixed(2)}</span></div>
              <div className="flex justify-between"><span>Total</span><span className="font-medium">${Number(totalTurno).toFixed(2)}</span></div>
              <div className="flex justify-between"><span>Ganancia Neta</span><span className="font-medium text-green-600">${Number(gananciaTurno).toFixed(2)}</span></div>

              <div className="mt-3">
                <p className="text-sm text-muted-foreground mb-1">Desglose por método (total ventas)</p>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(desgloseMetodo).map(([m, v]) => (
                    <div key={m} className="flex justify-between bg-background/50 rounded px-3 py-2 text-sm">
            ...
            [...state.cashClosures].reverse().map((closure, i) => (
              <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-background/50 p-3 rounded-lg text-sm">
                <div className="flex justify-between items-center">
                  ...
                  <div className={`font-bold ${Number(closure.difference || 0) === 0 ? 'text-green-500' : 'text-red-500'}`}>
                    Dif: ${Number(closure.difference || 0).toFixed(2)}
                  </div>
                  <Button size="icon" variant="ghost" onClick={() => printReport(closure)} className="h-8 w-8 text-muted-foreground">
                    <Printer size={16} />
                  </Button>
                </div>
              </motion.div>
